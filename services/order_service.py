        ...
        if order.payment_id:
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    response = await client.post(refund_url, json=refund_request, timeout=30.0)
                    response.raise_for_status()
                    logger.info(f"Payment refund successful for order {order_id}, payment {order.payment_id}")
            except httpx.HTTPError as e:
                logger.error(f"Payment refund failed for order {order_id}: {str(e)}")
