            ...
            try:
                async with httpx.AsyncClient() as client:
                    refund_response = await client.post(
                        f"{config.PAYMENT_SERVICE_URL}/payments/{order.payment_id}/refund",
                        json={
                    ...
                    logger.info(f"Refunded payment {order.payment_id} for order {order_id}, amount: {order.total}")
            except httpx.HTTPError as e:
                logger.error(f"Failed to refund payment {order.payment_id} for order {order_id}: {e}")
                raise ValueError(f"Payment refund failed: {e}") from e
            except Exception as e:
                logger.error(f"Unexpected error during payment refund for order {order_id}: {e}")
                raise ValueError(f"Payment refund failed: {e}") from e

        # Update order status after successful refund (or if no payment to refund)
