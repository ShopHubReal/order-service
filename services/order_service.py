        ...
        if order.payment_id:
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    logger.info(f"Payment refund initiated for order {order_id}, refund_id: {refund_data.get('refund_id')}")
            except httpx.HTTPStatusError as e:
                logger.error(f"Failed to refund payment for order {order_id}: {e}")
                raise ValueError(f"Payment refund failed: {e.response.status_code} - {e.response.text}")
