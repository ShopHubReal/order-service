        ...
        if order.payment_id:
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    refund_request = {
                        "transaction_ref": order.payment_id,
                        "reversal_notes": f"Order {order_id} cancelled by user {user_id}"
                    }
                    response = await client.post(refund_url, json=refund_request, timeout=10.0)
                    response.raise_for_status()

                    refund_data = response.json()
                    logger.info(
                        f"Refund processed for order {order_id}: "
                        f"refund_id={refund_data.get('id')}, "
                        f"status={refund_data.get('status')}"
                    )
            except httpx.HTTPError as e:
                logger.error(f"Failed to refund payment for order {order_id}: {str(e)}")
                raise ValueError(f"Payment refund failed: {str(e)}")
            except Exception as e:
                logger.error(f"Unexpected error during refund for order {order_id}: {str(e)}")
                raise
