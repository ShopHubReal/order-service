...
class OrderService:
    ...
    async def cancel_order(self, order_id: UUID, user_id: UUID) -> Order:
        ...
        # Refund payment if order has a payment_id
        if order.payment_id:
            try:
                ...
                logger.error(f"Failed to refund payment {order.payment_id} for order {order_id}: {e.response.text}")
                raise RuntimeError(f"Payment refund failed: {e.response.status_code}")
            except Exception as e:
                logger.error(f"Error refunding payment {order.payment_id} for order {order_id}: {str(e)}")
