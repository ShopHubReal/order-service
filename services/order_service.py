...
class OrderService:
    ...
    async def _refund_payment(self, payment_id: UUID, amount: Decimal) -> None:
        ...
        """
        timeout = 10.0  # Longer timeout for payment operations
        ...
        try:
            ...
            logger.error(f"Payment service error during refund: {e}")
            if e.response.status_code == 404:
                raise ValueError(f"Payment {payment_id} not found")
            elif e.response.status_code == 400:
                raise ValueError(f"Invalid refund request: {e.response.text}")
            else:
                raise
        except httpx.TimeoutException:
            logger.error(f"Payment service timeout during refund for payment {payment_id}")
            raise ValueError("Payment service timeout during refund")
        except Exception as e:
            logger.error(f"Unexpected error during payment refund: {e}")
            raise
