...
class OrderService:
    ...
    async def cancel_order(self, order_id: UUID, user_id: UUID) -> Order:
        ...
        self.db.commit()
        self.db.refresh(order)
        logger.info(f"Cancelled order {order_id} and processed refund for payment {order.payment_id}")
    ...
    async def _refund_payment(self, payment_id: UUID, amount: Decimal) -> None:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                logger.info(f"Successfully refunded payment {payment_id}: {refund_data}")
        except httpx.HTTPStatusError as e:
            logger.error(f"Payment refund failed for {payment_id}: {e.response.status_code} - {e.response.text}")
            raise ValueError(f"Payment refund failed: {e.response.status_code}")
        except Exception as e:
            logger.error(f"Error processing payment refund for {payment_id}: {e}")
            raise
