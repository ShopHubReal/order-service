...
class OrderService:
    ...
    async def cancel_order(self, order_id: UUID, user_id: UUID) -> Order:
        ...
        # Request refund from payment service if payment exists
        if order.payment_id:
            try:
                async with httpx.AsyncClient(timeout=15.0) as client:
                    ...
                    logger.info(f"Refund initiated for order {order_id}: {refund_data.get('id')}")
            except httpx.HTTPStatusError as e:
                error_detail = e.response.json().get("detail", str(e))
                logger.error(f"Refund failed for order {order_id}: {error_detail}")
                raise Exception(f"Failed to process refund: {error_detail}")
            except Exception as e:
                logger.error(f"Error refunding payment for order {order_id}: {e}")
                raise
