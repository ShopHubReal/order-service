    ...
    async def cancel_order(self, order_id: UUID, user_id: UUID) -> Order:
        ...
        if order.payment_id:
            try:
                ...
                logger.info(f"Payment refund initiated for order {order_id}, payment {order.payment_id}, refund_id: {refund_data.get('refund_id')}")

            except httpx.HTTPStatusError as e:
                logger.error(f"Failed to refund payment {order.payment_id} for order {order_id}: {e.response.status_code} - {e.response.text}")
                raise ValueError(f"Failed to process refund: {e.response.text}")
            except httpx.RequestError as e:
                logger.error(f"Payment service unavailable when refunding order {order_id}: {e}")
