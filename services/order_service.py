...
class OrderService:
    ...
    async def cancel_order(self, order_id: UUID, user_id: UUID) -> Order:
        ...
        # Refund the payment if payment_id exists
        if order.payment_id:
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    refund_request = {
                        ...
                        "reversal_notes": f"Order {order_id} cancelled by user {user_id}"
                    }
                    response = await client.post(refund_url, json=refund_request)
                ...
                raise ValueError(f"Failed to process refund for order {order_id}")

        order.status = "cancelled"
        self.db.commit()
        self.db.refresh(order)
        logger.info(f"Cancelled order {order_id} with refund processed")
