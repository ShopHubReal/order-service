...
class OrderService:
    ...
    async def cancel_order(self, order_id: UUID, user_id: UUID) -> Order:
        ...
        self.db.commit()
        self.db.refresh(order)
        logger.info(f"Cancelled order {order_id} and processed refund")
    ...
    async def _refund_payment(self, payment_id: UUID, amount: Decimal) -> None:
        ...
        """
        refund_url = f"{config.PAYMENT_SERVICE_URL}/v1/payments/{payment_id}/refund"
        refund_data = {
            "amount": str(amount),
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                logger.info(f"Successfully refunded payment {payment_id}, amount: {amount}")
            except httpx.HTTPStatusError as e:
                logger.error(f"Payment refund failed for {payment_id}: {e.response.text}")
                raise Exception(f"Payment refund failed: {e.response.status_code}")
            except httpx.RequestError as e:
                logger.error(f"Payment service request failed for {payment_id}: {e}")
