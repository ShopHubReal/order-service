            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    if refund_response.status_code != 200:
                        error_msg = f"Payment refund failed for order {order_id}: {refund_response.status_code}"
                        logger.error(f"{error_msg} - {refund_response.text}")
                        raise RuntimeError(error_msg)

                    refund_data = refund_response.json()
                    logger.info(f"Payment refund processed for order {order_id}: {refund_data.get('refund_id')}")

            except httpx.RequestError as e:
                error_msg = f"Failed to connect to payment service for refund: {str(e)}"
