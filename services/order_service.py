...
class OrderService:
    ...
    async def cancel_order(self, order_id: UUID, user_id: UUID) -> Order:
        ...
        if order.payment_id:
            try:
                ...
                # Re-raise the exception to prevent order cancellation if refund fails
                raise Exception(f"Payment refund failed: {str(e)}")
        else:
            logger.warning(f"Order {order_id} has no payment_id, skipping refund")
    ...
    async def _refund_payment(self, payment_id: UUID, amount: Decimal) -> None:
        """
        Refund payment via payment service API.
        ...
        Raises:
            Exception: If refund API call fails
        """
        refund_url = f"{config.PAYMENT_SERVICE_URL}/api/payments/{payment_id}/refund"
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                logger.info(f"Payment refund successful: {refund_result}")

            except httpx.HTTPStatusError as e:
                logger.error(f"HTTP error during payment refund: {e.response.status_code} - {e.response.text}")
                raise Exception(f"Payment service returned {e.response.status_code}: {e.response.text}")
            except httpx.RequestError as e:
                logger.error(f"Request error during payment refund: {str(e)}")
                raise Exception(f"Failed to connect to payment service: {str(e)}")
            except Exception as e:
                logger.error(f"Unexpected error during payment refund: {str(e)}")
                raise Exception(f"Payment refund failed: {str(e)}")
