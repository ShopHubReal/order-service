...
class OrderService:
    ...
    async def cancel_order(self, order_id: UUID, user_id: UUID) -> Order:
        ...
        if order.payment_id:
            await self._refund_payment(order.payment_id, order.total, order_id)
    ...
    async def _refund_payment(self, payment_id: UUID, amount: Decimal, order_id: UUID) -> None:
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                logger.info(f"Payment refunded: {payment_id}, refund_id: {refund_data.get('refund_id')}")
