        ...
        if order.payment_id:
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    logger.info(f"Refunded payment {order.payment_id} for order {order_id}: {refund_data}")
            except httpx.HTTPError as e:
                logger.error(f"Failed to refund payment {order.payment_id} for order {order_id}: {e}")
                raise httpx.HTTPError(f"Payment refund failed: {e}")
            except Exception as e:
                logger.error(f"Unexpected error during payment refund for order {order_id}: {e}")
                raise ValueError(f"Payment refund failed: {e}")
