    ...
    async def _refund_payment(self, payment_id: UUID, amount: Decimal) -> None:
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                logger.info(f"Successfully refunded payment {payment_id} for amount {amount}")

            except httpx.HTTPStatusError as e:
                error_msg = f"Payment refund failed for {payment_id}: HTTP {e.response.status_code}"
                logger.error(error_msg)
                raise Exception(error_msg)
            except httpx.RequestError as e:
                error_msg = f"Payment refund request failed for {payment_id}: {str(e)}"
                logger.error(error_msg)
                raise Exception(error_msg)
